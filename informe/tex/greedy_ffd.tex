...
\begin{itemize}
    \item Ordenar las habilidades: $\mathcal{O}(n \log n)$.
    \item Evaluar, para cada uno de los $n$ maestros, el aumento de costo en cada uno de los $k$ grupos: $\mathcal{O}(nk)$.
\end{itemize}
...
Para valores prácticos en los que $k \ll n$, el término dominante es $\mathcal{O}(n \log n)$, mientras que para $k$ grande el término lineal en $k$ domina. La complejidad espacial es $\mathcal{O}(n + k)$.
...
Backtraking. Como se vio anterioriormente, la solución Greedy tiene un orden $n.log(n)$
...
De manera análoga al análisis realizado para el método Greedy anterior,
la siguiente figura evalúa la calidad de las soluciones obtenidas por el
método Greedy-FFD.

Se presentan las mismas métricas:
 distribución del ratio Greedy/Óptimo,
 estadísticos de su evolución y diferencia absoluta entre ambas soluciones.

\begin{figure}[H]
    \centering
    \includegraphics[width=.8\textwidth]{img/ratio_ffd_bt_1.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=.8\textwidth]{img/ratio_ffd_bt_2.png}
\end{figure}

\begin{figure}[H]
    \centering
    \includegraphics[width=.8\textwidth]{img/ratio_ffd_bt_3.png}
\end{figure}

Los gráficos muestran que Greedy-FFD también produce soluciones muy cercanas al
óptimo: los ratios se mantienen cercanos a /textbf{1} y las diferencias absolutas
quedan acotadas en todos los tamaños evaluados.
